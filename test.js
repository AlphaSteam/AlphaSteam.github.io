...
import { GUI, controllers } from 'three/examples/jsm/libs/dat.gui.module.js';
...
var DEMO = {
    ...
    Initialize: function () {
        ...
        this.ms_Camera = new THREE.PerspectiveCamera(55.0, window.innerWidth / window.innerHeight, 0.7, 30000);
        this.ms_Camera.position.set(0, 0, 6);
        ...
        // geometry
        var width = 5;  // width of plane in world units
        var height = 5; // height of plane in world units
        ...
        this.uniforms = {
            ...
            u_mouse: { value: new THREE.Vector2(interpoint.x, interpoint.y) },
            ...
            red:{value: controls.Red},
            green:{value: controls.Green},
            blue:{value: controls.Blue}
...
void main() {
    ...
    vec3 aura = auraColor*m_dist*auraIntensity;
        color+=aura;
        color.r += dot(m_point,rColor) + red;
        color.g += dot(m_point,gColor) + green;
        color.b += dot(m_point,bColor) + blue;
    ...
    gl_FragColor = vec4(color,1.0);
        ...
        var presetsraw = fs.readFileSync('presets.json');
        ...
        redf.add(controls,'Red_x',0.0,1.0).step(0.01);
        redf.add(controls,'Red_y',0.0,1.0);

        var greenf = gui.addFolder('Green');
        greenf.add(controls,'Green_x',0,1).step(0.01);
        greenf.add(controls,'Green_y',0,1).step(0.01);

        var bluef = gui.addFolder('Blue');
        bluef.add(controls,'Blue_x',0,1).step(0.01);
        bluef.add(controls,'Blue_y',0,1).step(0.01);

        var postf = gui.addFolder('RGB post calculations');
        postf.add(controls,'Red',-1.0,1.0).step(0.01);
        postf.add(controls,'Green',-1.0,1.0).step(0.01);
        postf.add(controls,'Blue',-1.0,1.0).step(0.01);
        

        var intensityf = gui.addFolder('Intensity controller');
        intensityf.add(controls,'Intensity_control_x',-1,1).step(0.01);
        intensityf.add(controls,'Intensity_control_y',-1,1).step(0.01);

        var auraf = gui.addFolder('Aura');
        auraf.addColor(controls,'Aura_color');
        auraf.add(controls,'Aura_intensity',-2,2).step(0.01);

        var mousef = gui.addFolder('Mouse');
        mousef.add(controls,"Mouse_interaction");
        mousef.add(controls,"Mouse_scale",0.1,4).step(0.01);

        gui.add(controls, 'Speed', 0, 0.25).step(0.01);
        gui.add(controls, 'Scale',0.1,90).step(0.01);
        gui.add(controls,'Minimum_distance',0.1,2).step(0.01);
    ...
    Update: function () {
        ...
        this.uniforms.red.value =controls.Red;
        this.uniforms.green.value=controls.Green;
        this.uniforms.blue.value=controls.Blue;
        this.Display();
